"""
Create concrete regression figures from the saved experiment tables.

The figures follow the result layout used in the article: a degree-wise MSE
comparison, a quartic-family comparison, and a mixed-score view combining
normalized inverse MSE and normalized R2.
"""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "figures" / "concrete"

DEGREE_ORDER = ["linear", "quadratic", "cubic", "quartic"]
...
def _first_existing_path(paths: Sequence[Path]) -> Path:
    ...
    raise FileNotFoundError(f"No concrete result table was found among: {joined}")
...
def normalize_concrete_table(table: pd.DataFrame) -> pd.DataFrame:
    """Normalize common column names used by the regression scripts."""
    table = table.copy()
    dataset_col = _first_column(table, ["dataset", "dataset_name"])
    if dataset_col is not None:
        mask = table[dataset_col].astype(str).str.lower().str.contains("concrete")
...
def load_concrete_results(input_path: Optional[Path] = None) -> pd.DataFrame:
    """Load the concrete result table used by the plotting functions."""
    path = Path(input_path) if input_path is not None else _first_existing_path(DEFAULT_INPUTS)
    return normalize_concrete_table(pd.read_csv(path))
...
def build_degree_curve_table(table: pd.DataFrame) -> pd.DataFrame:
    """Create the degree-wise data used in the concrete trend plot."""
...
def build_quartic_comparison_table(table: pd.DataFrame) -> pd.DataFrame:
    """Create a compact quartic-family table for the concrete bar plot."""
...
def _plot_degree_curves(curves: pd.DataFrame, output_path: Path) -> Path:
    if curves.empty:
        raise ValueError("The concrete degree-curve table is empty.")
    ...
    ax.set_title("Concrete regression degree-wise comparison")
...
def create_concrete_figures(input_path: Optional[Path] = None, output_dir: Path = DEFAULT_OUTPUT_DIR) -> Dict[str, Path]:
    """Create the concrete CSV summaries and figure files."""
    output_dir = Path(output_dir)
    table = load_concrete_results(input_path)
    ...
    curves_path = output_dir / "02_concrete_degree_curves.csv"
    quartic_path = output_dir / "02_concrete_quartic_comparison.csv"
    ...
    paths = {
        ...
        "degree_curve_figure": _plot_degree_curves(curves, output_dir / "02_concrete_degree_mse.png"),
        "quartic_mse_figure": _plot_bar(quartic_scored, "mse", "Test MSE", "Concrete quartic-family comparison", output_dir / "02_concrete_quartic_mse.png"),
        "mixed_score_figure": _plot_bar(quartic_scored, "mixed_score_6_4", "Mixed score", "Concrete mixed-score comparison", output_dir / "02_concrete_mixed_score.png"),
    }
    return paths


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create concrete regression result figures.")
...
def main() -> None:
    args = parse_args()
    paths = create_concrete_figures(args.input, args.output_dir)
