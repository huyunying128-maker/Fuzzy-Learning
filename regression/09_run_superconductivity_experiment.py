"""
Superconductivity regression experiment runner.

The runner executes the superconductivity regression block in the same modular
format as the concrete experiment. It saves the global polynomial, crisp local,
fuzzy local, partition-only, fixed-iteration, modified k-means, and external-
regressor tables, then combines them into one dataset-level result file.
"""
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "regression" / "superconductivity"
...
_global = _load_regression_module("01_global_polynomial_baselines.py", "fpw_reg_global_sc")
_crisp = _load_regression_module("02_crisp_local_regression.py", "fpw_reg_crisp_sc")
_fuzzy = _load_regression_module("03_fuzzy_local_regression.py", "fpw_reg_fuzzy_sc")
_partition = _load_regression_module("04_partition_only_regression.py", "fpw_reg_partition_sc")
_fixed = _load_regression_module("05_fixed_iteration_regression.py", "fpw_reg_fixed_sc")
_modified = _load_regression_module("06_modified_kmeans_regression.py", "fpw_reg_modified_sc")
_external = _load_regression_module("07_external_regressors.py", "fpw_reg_external_sc")
...
def run_superconductivity_experiment(
    split_path: Path,
    ...
    """Run the superconductivity regression experiment and return the combined table."""
    output_dir = Path(output_dir)
    ...
    fixed_iterations = fixed_iterations if fixed_iterations is not None else [14]
    ...
    paths = {
        ...
        "combined": output_dir / "superconductivity_regression_all_results.csv",
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the superconductivity regression experiment block.")
    ...
    parser.add_argument("--fixed-iterations", default="14")
...
def main() -> None:
    args = parse_args()
    table = run_superconductivity_experiment(
    ...
    print(f"Saved superconductivity combined result table with {table.shape[0]} rows")
