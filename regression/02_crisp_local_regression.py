"""
Crisp partition-weighted local regression.

The crisp branch fits a hard nearest-centroid partition, constructs local inputs
from the hard membership table, fits local polynomial ridge models, and combines
local predictions through the corresponding aggregation weights.
"""
...
class CrispRegressionCandidate:
    """One evaluated crisp local-regression configuration."""
...
def evaluate_crisp_candidate(
    ...
    tolerance: float,
    random_state: int,
) -> Tuple[CrispRegressionCandidate, object, object]:
    """Fit one crisp local-regression candidate and evaluate validation error."""
    ...
    candidate = CrispRegressionCandidate(
        degree=int(degree),
    ...
    return candidate, partition, model


def refit_and_test_crisp_candidate(
    split: Dict[str, np.ndarray],
    selected: CrispRegressionCandidate,
    alpha: float,
    max_iter: int,
    tolerance: float,
    random_state: int,
) -> Dict[str, float]:
    """Refit the selected candidate on the non-test section and evaluate test data."""
...
def run_crisp_local_regression(
    split_path: Path,
    dataset_name: str,
    output_path: Path,
    ...
    """Search crisp local-regression candidates and save selected test rows."""
    split = load_split_npz(split_path)
    k_values, p_values = _candidate_grid(split["x_train"].shape[0], k_values_text, p_values_text)

    selected_rows: List[Dict[str, float]] = []
    search_records: List[Dict[str, float]] = []
    base_pairs = list(itertools.product(k_values, p_values))
    if max_candidates is not None:
        base_pairs = base_pairs[: int(max_candidates)]

    for degree in degrees:
        for truncation in truncations:
            best: Optional[CrispRegressionCandidate] = None
            for run_index, (k, p) in enumerate(base_pairs):
                candidate, _, _ = evaluate_crisp_candidate(
                    split["x_train"],
                    split["y_train"],
                    split["x_valid"],
                    split["y_valid"],
                ...
                record.update({"dataset": dataset_name, "method_family": "crisp_local"})
                search_records.append(record)
                if best is None or candidate.validation_mse < best.validation_mse:
                    best = candidate

            if best is None:
                raise RuntimeError("No crisp local-regression candidate was evaluated.")
            row = refit_and_test_crisp_candidate(
                split,
                selected=best,
                alpha=float(alpha),
                max_iter=int(max_iter),
            ...
            row.update(
                {
                    ...
                    "task": "regression",
                    "method_family": "crisp_local",
                    "method_name": "Crisp local regression",
                    "alpha": float(alpha),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fit crisp partition-weighted local regression.")
    parser.add_argument("--split", type=Path, required=True)
    parser.add_argument("--dataset-name", required=True)
    parser.add_argument("--output", type=Path, default=Path("outputs/regression/crisp_local_regression.csv"))
...
def main() -> None:
    args = parse_args()
    table = run_crisp_local_regression(
    ...
    print(f"Saved {table.shape[0]} crisp local-regression rows to {args.output}")
